...
static bool usbd_get_descriptor(uint16_t type_index, uint8_t **data, uint32_t *len)
{
    ...
    //USB_LOG_INFO("Get Desc: type=%d index=%d\n", type, index);
    ...
    if ((type == USB_DESCRIPTOR_TYPE_STRING)) {
		if((index == USB_OSDESC_STRING_DESC_INDEX)) {
			...
			p = (uint8_t *)g_usbd_core.msosv1_desc->string;
		}else{
			if(index >= g_usbd_core.string_cnt) {
				return false;
			}
			p = (uint8_t *)g_usbd_core.string_desc[index];
		}
        ...
        *len = g_usbd_core.bos_desc->string_len;
		...
		/*Invalid types of descriptors, see USB Spec. Revision 2.0, 9.4.3 Get Descriptor */
		return false;
	}
...
static bool check_endpoint(int ep)
{
	if(ep&0x80){
		return (g_usbd_core.tx_msg[ep&0x7f].ep==ep) ? true : false;
	}else{
		return (g_usbd_core.rx_msg[ep&0x7f].ep==ep) ? true : false;
	}
}
...
static bool usbd_set_configuration(uint8_t config_index, uint8_t alt_setting)
{
    ...
    bool found = false;
    uint8_t *p = (uint8_t *)g_usbd_core.descriptors;
    ...
    while (p[DESC_bLength] != 0U) {
        switch (p[DESC_bDescriptorType]) {
            ...
            case USB_DESCRIPTOR_TYPE_ENDPOINT:
				...
				// only open used endpoint.
				if(check_endpoint(p[2])){
...
__WEAK int usbd_vendor_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
	return 0;
}
...
static int usbd_vendor_request_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
	...
	return usbd_vendor_handler(setup, data, len);
}
...
void usbd_desc_register(const uint8_t *desc, uint8_t **string_desc, int string_cnt)
{
    memset(&g_usbd_core, 0, sizeof(struct usbd_core_priv));

    g_usbd_core.descriptors = desc;
	g_usbd_core.string_desc = string_desc;
	g_usbd_core.string_cnt = string_cnt;
